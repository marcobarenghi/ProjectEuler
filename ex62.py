...
base=1
while(True):
	#first, find the 5 cubes
	cubes.append(sorted(str(base**3)))
	if(cubes.count(cubes[-1])==nTarget):
		#then, proceed backward to find smallest cube
		count=1
		sortedCube=sorted(str(base**3))
		for b in range(base-1,-1,-1):
			if(sorted(str(b**3))==sortedCube):
				count+=1
				if(count==nTarget):
					print(b**3)
					break			
